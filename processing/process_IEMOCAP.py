...
def process(path, t=4, RATE=16000, impro_or_script='impro', without_silence=True, win_length=2048,hop_length=512,to_save_feature=True):
    path = path.rstrip('/')
    ...
    trans_file = '/home/liuluyao/IEMOCAP/trans_ids.plk'
    ...
    for leave in range(5):
        ...
        meta_dict = {}
        val_dict = {}
        features_file_name = './{}_features_{}_MFCC.pkl'.format('leave'+str(leave+1), 'IEMOCAP')
        ...
        for i, wav_file in enumerate(tqdm(train_files)):
            ...
            if (t * RATE >= len(wav_data)):
                ...
                x1 = list(wav_data[:int(t*RATE)])
                y1.append(label)

            if os.path.basename(wav_file).split('-')[3] !='F' and os.path.basename(wav_file).split('-')[3] != 'M':
                print(os.path.basename(wav_file).split('-')[3])
                print(os.path.basename(wav_file).split('-'))


            meta_dict[i] = {
                'X': x1,
                'trans':trans_feature[os.path.basename(wav_file)[:-4]],
                'trans_len':len([x for x in trans_feature[os.path.basename(wav_file)[:-4]] if x!=0]),
                'y': y1,
                'path': wav_file,
                'sex':os.path.basename(wav_file).split('-')[3]
            }
...
process(r'/home/liuluyao/IEMOCAP/wav')
