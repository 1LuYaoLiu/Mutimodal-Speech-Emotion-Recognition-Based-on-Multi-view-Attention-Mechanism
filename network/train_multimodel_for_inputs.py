...
#from model_MSP import MULTIMANET
...
def train(file_num, case, element):
    ...
    MODEL_PATH = './model_result/MSP/different_inputs/{}_input/{}_leave_{}_models.pth'.format(case,element, file_num)

    # load features file
    print('load features data ...')
    logging.info('load features data...')
    #
    file = r'processing/leave{}_features_MSP_MFCC.pkl'.format(file_num)
    ...
    # show the number of data(for training and validation)
    train_label_num = {
        0: 0,
        1: 0,
        2: 0,
    ...
    print('train_label_num:', train_label_num)
    print('val_label_num:', val_label_num)
    ...
    model = MULTIMANET(ahead_text=7, ahidden_text=96,ahead_audio=4, ahidden_audio=96, with_gender=True,case=4, element='1234')
    ...
    for i in range(Epoch):
        ...
        for _,data in enumerate(train_loader):
            ...
            if element == 'TTT':
                out_emotion, out_gender, out_emotion_center, out_gender_center = model(x_t.unsqueeze(1), x_t.unsqueeze(1),
                                                                                   x_t.unsqueeze(1), trans, trans_len)
            elif element == 'FFF':
                out_emotion, out_gender, out_emotion_center, out_gender_center = model(x_f.unsqueeze(1),
                                                                                       x_f.unsqueeze(1),
                                                                                       x_f.unsqueeze(1), trans,
                                                                                       trans_len)
            elif element == 'MMM':
                out_emotion, out_gender, out_emotion_center, out_gender_center = model(x_m.unsqueeze(1),
                                                                                       x_m.unsqueeze(1),
                                                                                       x_m.unsqueeze(1), trans,
                                                                                       trans_len)
            elif element == 'TFF':
                out_emotion, out_gender, out_emotion_center, out_gender_center = model(x_t.unsqueeze(1),
                                                                                       x_f.unsqueeze(1),
                                                                                       x_f.unsqueeze(1), trans,
                                                                                       trans_len)
            elif element == 'TFT':
        ...
        for i in range(len(val_y)):
            ...
            # , trans.unsqueeze(0), torch.tensor([trans_len]
            if element == 'TTT':
                out_emotion, out_gender, out_emotion_center, out_gender_center = model(x_t.unsqueeze(0).unsqueeze(0),
                                                                                  x_t.unsqueeze(0).unsqueeze(0),
                                                                                  x_t.unsqueeze(0).unsqueeze(0),
                                                                                  trans.unsqueeze(0), torch.tensor([trans_len]))
            if element == 'FFF':
                out_emotion, out_gender, out_emotion_center, out_gender_center = model(x_f.unsqueeze(0).unsqueeze(0),
                                                                                  x_f.unsqueeze(0).unsqueeze(0),
                                                                                  x_f.unsqueeze(0).unsqueeze(0),
                                                                                  trans.unsqueeze(0), torch.tensor([trans_len]))
            if element == 'MMM':
                out_emotion, out_gender, out_emotion_center, out_gender_center = model(x_m.unsqueeze(0).unsqueeze(0),
                                                                                  x_m.unsqueeze(0).unsqueeze(0),
                                                                                  x_m.unsqueeze(0).unsqueeze(0),
                                                                                  trans.unsqueeze(0), torch.tensor([trans_len]))
            if element == 'TFF':
                out_emotion, out_gender, out_emotion_center, out_gender_center = model(x_t.unsqueeze(0).unsqueeze(0),
        ...
        if (maxWA<WA):
            ...
            matrix_file = './model_result/MSP_input/{}_{}_matrix.plk'.format(element, file_num)
